...
class TorchTrainer(Trainer):
    ...
    def execute(self):
        ...
        for season in self.seasons:
            ...
            print(f"Image channels passed to model: {self.image_channels}")
            optimizer = optim.Adam(cvae_model.parameters(), lr=self.lr)

            # load training set and train data
            train_time = pd.read_csv(
                self.input_path + f"/dates_train_{season}_data_{self.n_memb}memb_mod1.csv"
            )
            train_data = np.load(
                self.input_path + f"/preprocessed_3d_train_{season}_data_{self.n_memb}memb_mod1.npy"
            )
            n_train = len(train_data)
            trainset = [
                (torch.from_numpy(np.reshape(train_data[i], (3, 32, 32))), train_time["0"][i])
            ...
            test_time = pd.read_csv(
                self.input_path + f"/dates_test_{season}_data_{self.n_memb}memb_mod1.csv"
            )
            test_data = np.load(
                self.input_path + f"/preprocessed_3d_test_{season}_data_{self.n_memb}memb_mod1.npy"
            )
        ...
        if valid_epoch_loss < self.min_valid_epoch_loss:
            ...
            torch.save(
                ...
                self.output_path + f"/cvae_model_MSEloss_{season}_3d_{self.n_memb}memb_mod1.pth",
            )
            ...
            pd.DataFrame(train_loss).to_csv(
                self.output_path + f"/train_loss_indiv_MSEloss_{season}_3d_{self.n_memb}memb_mod1.csv"
            )
            pd.DataFrame(valid_loss).to_csv(
                self.output_path + f"/test_loss_indiv_MSEloss_{season}_3d_{self.n_memb}memb_mod1.csv"
            )
