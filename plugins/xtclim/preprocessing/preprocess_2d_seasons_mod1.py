...
class SplitPreprocessedData(DataGetter):
    ...
    def season_split(
        ...
        np.save(
            self.input_path
            + f"/preprocessed_3d_{dataset_type}{scenario}_winter_data_{self.n_memb}memb_mod1.npy",
            winter_images,
        )
        np.save(
            self.input_path
            + f"/preprocessed_3d_{dataset_type}{scenario}_spring_data_{self.n_memb}memb_mod1.npy",
            spring_images,
        )
        np.save(
            self.input_path
            + f"/preprocessed_3d_{dataset_type}{scenario}_summer_data_{self.n_memb}memb_mod1.npy",
            summer_images,
        )
        np.save(
            self.input_path
            + f"/preprocessed_3d_{dataset_type}{scenario}_autumn_data_{self.n_memb}memb_mod1.npy",
            autumn_images,
        )
        pd.DataFrame(winter_time).to_csv(
            self.input_path + f"/dates_{dataset_type}_winter_data_{self.n_memb}memb_mod1.csv"
        )
        pd.DataFrame(spring_time).to_csv(
            self.input_path + f"/dates_{dataset_type}_spring_data_{self.n_memb}memb_mod1.csv"
        )
        pd.DataFrame(summer_time).to_csv(
            self.input_path + f"/dates_{dataset_type}_summer_data_{self.n_memb}memb_mod1.csv"
        )
        pd.DataFrame(autumn_time).to_csv(
            self.input_path + f"/dates_{dataset_type}_autumn_data_{self.n_memb}memb_mod1.csv"
        )
    ...
    def execute(self):
        ...
        train_images = np.load(self.input_path + "/preprocessed_3d_train_data_allssp_mod1.npy")
        test_images = np.load(self.input_path + "/preprocessed_3d_test_data_allssp_mod1.npy")
        ...
        for scenario in self.scenarios:
            proj_images = np.load(
                self.input_path + f"/preprocessed_3d_proj{scenario}_data_allssp.npy"
            )
